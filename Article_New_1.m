...
CV = avg_CV(find(avg_CV>max(avg_CV)-0.05*range(avg_CV))); % Here we considered top 5% constraint, DM is free change it
...
%% ARRENGING THE INDIVIDUALLY TRAINED OBJECTIVES AND PREPARING FOR PLOTTING U-MATRIX 
sMap_umatrix = sMap_F;
...
%% Visualization of SOM results( U Matrix and Objective Component Planes )
figure(1) 
...
pause % Strike any key to supply Pareto Optimal Solution corresponding to start point... 
...
%% HIGHLIGHTING NEXT FINAL SOLUTION
figure(1) 
som_show(sMap_umatrix,'umat',3:7,'comp',3:7);
som_show_add('hit',hcv,'Markersize',1,'MarkerColor',0.5*[1 1 1],'EdgeColor',0.25*[1 1 1],'Subplot',2:6);
som_show_add('hit',h,'Markersize',1,'MarkerColor','k','EdgeColor','k','Subplot',2:6);
